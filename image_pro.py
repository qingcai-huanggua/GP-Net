...
class Image:
    ...
    def suofang(self,target_height, target_width):
        '''
          :功能        ：等比例缩放，上面的缩放是通过裁剪实现，改变了原图状态，这个是等比例进行缩放，不改变原图状态
          :参数 factor ：缩放比例因子，比如设置为0.5就是裁剪原图50%的区域
          '''
        height, width = self.img.shape[:2]  # 取彩色图片的长、宽。
        ...
        self.img = cv2.resize(self.img, size, interpolation=cv2.INTER_AREA)  # 双线性插值
        # plt.figure(figsize = (10,10))
        # plt.imshow(shrink)
        # plt.show()
        # BLACK = [255, 255, 255]
...
class DepthImage(Image,object):
    ...
    def normalize(self):
        ...
        self.img = self.img.astype('float32') / self.img.max()
        self.img = self.img - self.img.mean()
    @classmethod
