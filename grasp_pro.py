...
class Grasp:
    ...
    #在ggcnn里面ｌｅｎｔｈ更重要一些，我换一下顺序，再看看效果
    ...
    def scale(self,factor_h,factor_w):
        ...
        self.points[0][1] *= factor_h
        self.points[0][0] *= factor_w
        self.points[1][1] *= factor_h
        self.points[1][0] *= factor_w
        self.points[2][1] *= factor_h
        self.points[2][0] *= factor_w
        self.points[3][1] *= factor_h
        self.points[3][0] *= factor_w
...
class Grasps:
    ...
    @classmethod
    def load_from_regrad_files(cls,regrad_grasp_files,scale_h,scale_w):
        grasp_rectangles = []

        with open(regrad_grasp_files,'r') as f:
        ...
        grasp_rectangles = cls(grasp_rectangles)
        grasp_rectangles.scale(0.5,0.5)
        #grasp_rectangles.scale(scale_h,scale_w)
...
class Grasp_cpaw:
    ...
    def as_gr(self):
        ...
        # return Grasp(np.array(
        #     [
        #      [(y1 - self.length/2 * yo)/2, (x1 - self.length/2 * xo)/2],
        #      [(y2 - self.length/2 * yo)/2, (x2 - self.length/2 * xo)/2],
        #      [(y2 + self.length/2 * yo)/2, (x2 + self.length/2 * xo)/2],
        #      [(y1 + self.length/2 * yo)/2, (x1 + self.length/2 * xo)/2],
        #      ]
        # ).astype(np.float))
