...
def run(net):
    ...
    #准备数据集
    #训练集
    train_data = Regrad('/home/user/xyh_rl_projects/new_data')
    train_dataset = torch.utils.data.DataLoader(train_data,batch_size = batch_size,shuffle = True)
    #验证集
    val_data = Regrad('/home/user/xyh_rl_projects/new_data')
...
def visualization(val_data,idx,grasps_pre,grasps_true):
    ...
    img.crop((top,left),(top+300,left+300))
...
if __name__ == '__main__':
    net = GGCNN(4)
    run(net)
    torch.save(net,'trained_models/model_v1')